...
inline void scheduleLayerScrollProbe(QApplication& app, const QString& folder, Position target)
{
    ...
    auto wheel = [report](int delta, Qt::KeyboardModifiers modifiers, bool zoom) {
        ...
        // Forward-projected Y of ANY fixed world tile differs only by this
        // origin. At zoom 3 the old path moved it by 60 screen pixels per step.
        const int canvasDelta = renderer->moveY()+20*afterLevel-(beforeY+20*beforeLevel);
        const bool pass = afterLevel == expected && renderer->moveX() == beforeX
            && canvasDelta == 0 && (zoom ? renderer->scale()!=beforeScale : renderer->scale()==beforeScale);
    ...
    QTimer::singleShot(8500,&app,[folder,report,wheel,target]() {
        ...
        int changed = 0;
        // The fixture opens the upper layer over this lower-level rock patch.
        // It must stay at the same framebuffer pixels when the cutaway lowers.
        const QRect patch(before.width()/2-60,before.height()/2-25,120,50);
        ...
        report(QString("%1 framebuffer stationary terrain changedPixels=%2/6000")

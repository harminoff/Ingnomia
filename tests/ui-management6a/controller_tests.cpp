...
int main()
{
	...
	StockpileSnapshot sp; sp.id={20}; sp.name="Stone";
	...
	c.setSearch(""); c.toggleSelectedStockpileFilter();
	check(port.actions.back().id.value=="stockpile.set_filter"&&!std::get<SetStockpileFilterPayload>(port.actions.back().payload).active,"stockpile filter mutation");
	c.setStockpileBasics("Stone II",2,true,true,false);
	check(port.actions.back().id.value=="stockpile.set_basics","stockpile basics mutation");
	...
	c.setStockpileTemplateName("food BASICS"); c.saveStockpileTemplate();
	...
	c.setStockpileBasics("Blocked during prompt",2,true,true,false);
	...
	c.setStockpileTemplateName("Building stone"); c.saveStockpileTemplate(); c.cancelStockpileTemplateOverwrite();
	...
	check(c.state().agriculture.pane==AgriculturePane::Overview,"new farm opens Overview");
	c.setAgriculturePane(AgriculturePane::Products);check(c.state().agriculture.pane==AgriculturePane::Products,"Products sidebar opens its pane");
	...
	farm.ready=4;c.showAgriculture(farm,Revision{3},WorldPosition{8,9,10});check(c.state().agriculture.pane==AgriculturePane::Products&&c.state().agriculture.value.ready==4&&c.state().agriculture.selectedPlots.size()==2,"live Farm updates preserve the pane and selected plots");
	farm.target.designation=DesignationId{32};c.showAgriculture(farm,Revision{4},WorldPosition{9,9,10});check(c.state().agriculture.pane==AgriculturePane::Overview,"new designation resets to Overview");

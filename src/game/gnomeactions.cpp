...
BT_RESULT Gnome::actionClaimItems( bool halt )
{
    ...
    {
        auto* ws=g->wsm()->workshopAt(m_job->pos());
        for(const auto& component : m_job->requiredItems()) {
            const auto items=g->inv()->getWorkshopItems(ws?ws->inputPos():m_job->pos(),component.itemSID,
                component.materialSID,component.count,component.requireSame,component.materialRestriction,
                ws?ws->linkedStockpiles():QList<unsigned int>{});
            if(items.size()!=component.count) { unclaimAll(); return BT_RESULT::FAILURE; }
            for(auto id : items) addClaimedItem(id,m_job->id());
        }
    }

...
void InspectorController::showStockpile( StockpileInspectorState value ) { if(!state_.acceptsWorldActions||!value.id)return;auto pos=state_.selected?state_.selected->position:std::nullopt;select({state_.world,EntityKind::Stockpile,value.id.value,pos},InspectorKind::Stockpile);state_.stockpile=std::move(value);state_.pendingAction.reset();notify(); }
...
void InspectorController::toggleStockpileSuspended(){if(!state_.stockpile || state_.pendingAction)return;const auto&s=*state_.stockpile;dispatch("stockpile.set_basics",SetStockpileBasicsPayload{s.id,s.name,s.priority,!s.suspended,s.pullFromOthers,s.allowPullFromHere});}

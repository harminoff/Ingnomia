...
HudRmlBinding::HudRmlBinding( Rml::Context& context ) : context_( context ) {}
HudRmlBinding::~HudRmlBinding() { shutdown(); }
bool HudRmlBinding::initialize( HudController& controller )
{
	controller_ = &controller; document_ = context_.LoadDocument( "screens/game_hud.rml" ); if( !document_ ) return false; localization::applyRmlText( *document_, textCatalog_ );
	...
	bind("hud_tool_mine",[this]{const bool open=!mineMenuOpen_;toggleActionMenu("mine");if(open)controller_->activateTool(ToolId{"mine"});stateChanged(controller_->state());});
	bind("hud_tool_build",[this]{buildMenuOpen_=!buildMenuOpen_;if(buildMenuOpen_)controller_->requestBuildItems(BuildSelection::Furniture,{});else controller_->closeBuildMenu();stateChanged(controller_->state());});
	bind("hud_tool_agriculture",[this]{toggleActionMenu("agriculture");});
	bind("hud_tool_designations",[this]{toggleActionMenu("designations");});
	bind("hud_tool_jobs",[this]{toggleActionMenu("jobs");});
	...
	bind("hud_mine_walls",[this]{selectMineMode("mine");}); bind("hud_mine_explorative",[this]{selectMineMode("explorative_mine");}); bind("hud_mine_remove_floor",[this]{selectMineMode("remove_floor");}); bind("hud_mine_hole",[this]{selectMineMode("dig_hole");}); bind("hud_mine_stairs_down",[this]{selectMineMode("dig_stairs_down");}); bind("hud_mine_stairs_up",[this]{selectMineMode("mine_stairs_up");}); bind("hud_mine_ramp_down",[this]{selectMineMode("dig_ramp_down");}); bind("hud_mine_close",[this]{mineMenuOpen_=false;stateChanged(controller_->state());});
	bind("hud_agriculture_close",[this]{agricultureMenuOpen_=false;stateChanged(controller_->state());}); bind("hud_designations_close",[this]{designationMenuOpen_=false;stateChanged(controller_->state());}); bind("hud_jobs_close",[this]{jobsMenuOpen_=false;stateChanged(controller_->state());});
	bind("hud_build_deconstruct",[this]{controller_->activateTool(ToolId{"deconstruct"});stateChanged(controller_->state());}); bind("hud_build_close",[this]{const auto active=controller_->state().tool.active;const bool buildActive=active&&active->value=="build";buildMenuOpen_=false;controller_->closeBuildMenu();if(buildActive)controller_->cancelTool();stateChanged(controller_->state());});
	...
	bindSidebarTooltip("hud_tool_build","hud.tip.build"); bindSidebarTooltip("hud_tool_mine","hud.tip.mine"); bindSidebarTooltip("hud_tool_agriculture","hud.tip.agriculture"); bindSidebarTooltip("hud_tool_designations","hud.tip.designations"); bindSidebarTooltip("hud_tool_jobs","hud.tip.jobs");
	...
	bindEvent("hud_build_catalog","click",[this](Rml::Event& event){if(updatingBuildCatalog_){event.StopPropagation();return;}for(auto* element=event.GetTargetElement();element&&element!=event.GetCurrentElement();element=element->GetParentNode()){if(!element->GetAttribute<Rml::String>("data-build-component","").empty()){event.StopPropagation();return;}const auto id=element->GetAttribute<Rml::String>("data-build","");if(!id.empty()){selectedBuild_=id;const auto action=element->GetAttribute<Rml::String>("data-build-action","");const auto buildAction=action=="FillHole"?BuildAction::FillHole:action=="Replace"?BuildAction::Replace:BuildAction::Build;controller_->chooseBuildAction(CatalogId{id},buildAction);event.StopPropagation();stateChanged(controller_->state());break;}}});
...
void HudRmlBinding::showSidebarTooltip( const char* textKey, Rml::Element* source )
{
	...
	if ( auto* tooltip = document_->GetElementById( "hud_sidebar_tooltip" ) )
	{
		...
		tooltip->SetProperty( "left", std::to_string( offset.x + size.x + 10.0f ) + "dp" );
		tooltip->SetProperty( "top", std::to_string( offset.y + size.y * 0.5f ) + "dp" );
...
void HudRmlBinding::selectBuildCategory(const char* elementId, BuildSelection selection, std::string_view category){selectedBuildCategory_=elementId;selectedBuild_.clear();buildMenuOpen_=true;controller_->requestBuildItems(selection,category);stateChanged(controller_->state());}
...
void HudRmlBinding::stateChanged( const HudState& s )
{
	...
	visible("hud_mine_menu",mineMenuOpen_ && s.acceptsWorldActions);
	visible("hud_agriculture_menu",agricultureMenuOpen_ && s.acceptsWorldActions);
	visible("hud_designations_menu",designationMenuOpen_ && s.acceptsWorldActions);
	visible("hud_jobs_menu",jobsMenuOpen_ && s.acceptsWorldActions);
	visible("hud_kingdom_panel",kingdomPanelOpen_ && s.acceptsWorldActions);
	visible("hud_build_panel",s.acceptsWorldActions && buildMenuOpen_);
	visible("hud_build_categories",s.acceptsWorldActions && buildMenuOpen_);
	visible("hud_build_catalog",s.acceptsWorldActions && buildMenuOpen_);
	...
	{
		const bool show=s.acceptsWorldActions && buildMenuOpen_;
		panel->SetProperty("display",show?"block":"none");
	}
	if(auto* catalog=document_->GetElementById("hud_build_catalog"); renderedBuildCatalog_ != s.buildCatalog || renderedSelectedBuild_ != selectedBuild_)
	{
		...
		if ( s.buildCatalog.empty() ) markup += "<p class='c-hud-build-empty'>" + Rml::StringUtilities::EncodeRml( textCatalog_.format( LocalizationKey{ "hud.build.no_items" } ) ) + "</p>";
		for(const auto& row:s.buildCatalog)
		{
			...
			if ( !row.components.empty() )
			{
				...
				for ( std::size_t componentIndex = 0; componentIndex < row.components.size(); ++componentIndex )
				{
					...
					markup += std::string{ "<label class='c-hud-build-requirement'><span>" } + std::to_string( component.amount ) + " x " + Rml::StringUtilities::EncodeRml( component.item.value ) + "</span>";
					...
					else
					{
						...
						for ( const auto& option : component.options )
							markup += "<option value='" + Rml::StringUtilities::EncodeRml( option.id.value ) + "'" + ( option.id == component.selected ? " selected" : "" ) + ">" + Rml::StringUtilities::EncodeRml( option.id.value ) + " (" + std::to_string( option.available ) + ")</option>";
						markup += "</select>";
					}
					markup += "</label>";
				}
			...
			if ( row.kind == BuildKind::Terrain )
			{
				markup += "<button id='hud_build_action_FillHole_" + domId( "", row.id.value ) + "' class='c-button c-hud-build-action' data-build='" + encodedId + "' data-build-action='FillHole'" + ( row.available ? "" : " disabled='disabled' aria-disabled='true'" ) + ">Fill hole</button><button id='hud_build_action_Replace_" + domId( "", row.id.value ) + "' class='c-button c-hud-build-action' data-build='" + encodedId + "' data-build-action='Replace'" + ( row.available ? "" : " disabled='disabled' aria-disabled='true'" ) + ">Replace</button>";
			}
			const auto buildLabel = textCatalog_.format( LocalizationKey{ "hud.build.action_build" } );
			markup += "<button id='hud_build_action_Build_" + domId( "", row.id.value ) + "' class='c-button c-hud-build-action' data-build='" + encodedId + "' data-build-action='Build' aria-label='" + Rml::StringUtilities::EncodeRml( buildLabel ) + "'" + ( row.available ? "" : " disabled='disabled' aria-disabled='true'" ) + ">" + Rml::StringUtilities::EncodeRml( buildLabel ) + "</button></div>";
			markup += "</article>";
		}
		markup += "</div>";
	...
	for(const auto* id:{"hud_build_furniture","hud_build_workshop","hud_build_containers","hud_build_utility","hud_build_wall","hud_build_floor","hud_build_stairs","hud_build_ramps","hud_build_fence"})if(auto* element=document_->GetElementById(id))element->SetClass("is-selected",selectedBuildCategory_==id);
	...
	text("hud_active_tool",s.tool.active?s.tool.active->value:tr("hud.inspect"));text("hud_rotate_hint",s.tool.canRotate?tr("hud.rotate_hint"):"");

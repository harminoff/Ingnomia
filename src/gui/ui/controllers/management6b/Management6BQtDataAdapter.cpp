...
template<class I> void itemDetail(InventoryRow& out,const I& source)
{
	for(const auto& recipe:source.madeBy)
	{
		ItemRecipe row{text(recipe.id),text(recipe.outputItemID),displayName(recipe.outputName,recipe.outputItemID),text(recipe.workshop),text(recipe.skill),recipe.amount,{}};
		for(const auto& ingredient:recipe.ingredients)
			row.ingredients.push_back({text(ingredient.itemID),displayName(ingredient.name,ingredient.itemID),text(ingredient.allowedMaterial),text(ingredient.allowedMaterialType),ingredient.amount});
		out.madeBy.push_back(std::move(row));
	}
	for(const auto& recipe:source.usedIn)
	{
		ItemRecipe row{text(recipe.id),text(recipe.outputItemID),displayName(recipe.outputName,recipe.outputItemID),text(recipe.workshop),text(recipe.skill),recipe.amount,{}};
		for(const auto& ingredient:recipe.ingredients)
			row.ingredients.push_back({text(ingredient.itemID),displayName(ingredient.name,ingredient.itemID),text(ingredient.allowedMaterial),text(ingredient.allowedMaterialType),ingredient.amount});
		out.usedIn.push_back(std::move(row));
	}
	for(const auto& location:source.locations)out.locations.push_back({location.id,text(location.name),location.count});
}
...
std::vector<SkillCatalogRow> Management6BQtDataAdapter::skillCatalog(const QList<GuiSkillInfo>& values) const
{
	std::vector<SkillCatalogRow> rows; rows.reserve(values.size());
	for(const auto& value:values) rows.push_back({CatalogId{text(value.sid)},text(value.name),text(value.group)});
	return rows;
}
std::pair<ProfessionId,std::vector<CatalogId>> Management6BQtDataAdapter::professionSkills(const QString&profession,const QList<GuiSkillInfo>&skills)const{std::vector<CatalogId> out;out.reserve(skills.size());for(const auto&s:skills)out.push_back(CatalogId{text(s.sid)});return{ProfessionId{text(profession)},std::move(out)};}
...
Snapshot<std::vector<InventoryRow>> Management6BQtDataAdapter::inventory(const QList<GuiInventoryCategory>&i){std::vector<InventoryRow> rows;for(const auto&c:i){InventoryRow cr;cr.id=rowId(c.id,{},{},{},InventoryDepth::Category);cr.name=displayName(c.name,c.id);totals(cr,c);rows.push_back(cr);for(const auto&g:c.groups){InventoryRow gr;gr.id=rowId(c.id,g.id,{},{},InventoryDepth::Group);gr.name=displayName(g.name,g.id);totals(gr,g);rows.push_back(gr);for(const auto&it:g.items){InventoryRow ir;ir.id=rowId(c.id,g.id,it.id,{},InventoryDepth::Item);ir.name=displayName(it.name,it.id);totals(ir,it);itemDetail(ir,it);ir.spriteSheet=text(it.spriteSheet);ir.spriteX=it.spriteX;ir.spriteY=it.spriteY;ir.spriteWidth=it.spriteWidth;ir.spriteHeight=it.spriteHeight;ir.spriteSheetWidth=it.spriteSheetWidth;ir.spriteSheetHeight=it.spriteSheetHeight;rows.push_back(ir);for(const auto&m:it.materials){InventoryRow mr;mr.id=rowId(c.id,g.id,it.id,m.id,InventoryDepth::Material);mr.name=displayName(m.name,m.id);totals(mr,m);itemDetail(mr,m);mr.spriteSheet=text(m.spriteSheet);mr.spriteX=m.spriteX;mr.spriteY=m.spriteY;mr.spriteWidth=m.spriteWidth;mr.spriteHeight=m.spriteHeight;mr.spriteSheetWidth=m.spriteSheetWidth;mr.spriteSheetHeight=m.spriteSheetHeight;rows.push_back(mr);}}}}return{world_,Revision{++inventoryRevision_.value},std::move(rows)};}

...
CommandResult InspectorQtCommandPort::dispatch(const UiActionEnvelope&a)
{
	...
	// Management windows own their subscriptions independently of the inspector.
	if(a.id.value=="inspect.clear")return queue([c=connector_]{if(!c)return;c->aggregatorCreatureInfo()->onRequestCreatureUpdate(0);c->aggregatorTileInfo()->clearSelection();});
	...
	if(a.id.value=="workshop.set_basics"){const auto*p=std::get_if<SetWorkshopBasicsPayload>(&a.payload);if(!p)return reject("ui.error.invalid_payload");const bool linked=p->linkStockpile.value_or(p->connectStockpile.has_value()||workshopLinks_.value(p->workshop.value,false));return queue([c=connector_,v=*p,linked]{if(!c)return;auto*a=c->aggregatorWorkshop();a->onSetBasicOptions(v.workshop.value,QString::fromStdString(v.name),v.priority,v.suspended,v.acceptGenerated,v.autoCraftMissing,linked,!v.linkStockpile.has_value() && !v.connectStockpile.has_value());a->onUpdateWorkshopInfo(v.workshop.value);});}
